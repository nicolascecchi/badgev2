...
NUM_QUERY_VAL = int(np.floor(NUM_QUERY * 0.3))
NUM_ROUND = int((opts.nEnd - NUM_INIT_LB)/ opts.nQuery)
DATA_NAME = opts.data
NUM_TOTAL_AL_QUERY = NUM_INIT_LB + NUM_ROUND * NUM_QUERY
...
idxs_lb = np.zeros(n_pool, dtype=bool) # Everything in false
idxs_tmp = np.arange(n_pool) # Indexes
np.random.shuffle(idxs_tmp) # Shuffle indexes
idxs_lb[idxs_tmp[:NUM_INIT_LB]] = True # change to TRUE the values at idxs_lb at positions of the first NUM_INIT_LB of shuffled indexes

idxs_val = np.zeros(n_pool, dtype=bool) # Everything in false
QVAL = int(np.floor(0.3 * NUM_INIT_LB))
idxs_val[idxs_tmp[NUM_INIT_LB:NUM_INIT_LB+QVAL]] = True
...
if opts.alg == 'rand': # random sampling
    strategy = RandomSampling(X_tr, Y_tr, idxs_lb, idxs_val, net, handler, args)
elif opts.alg == 'bait': # bait sampling
    strategy = BaitSampling(X_tr, Y_tr, idxs_lb, idxs_val, net, handler, args)
elif opts.alg == 'conf': # confidence-based sampling
    strategy = LeastConfidence(X_tr, Y_tr, idxs_lb, idxs_val, net, handler, args)
elif opts.alg == 'marg': # margin-based sampling
    strategy = MarginSampling(X_tr, Y_tr, idxs_lb, idxs_val, net, handler, args)
elif opts.alg == 'badge': # batch active learning by diverse gradient embeddings
    strategy = BadgeSampling(X_tr, Y_tr, idxs_lb, idxs_val, net, handler, args)
elif opts.alg == 'coreset': # coreset sampling
    strategy = CoreSet(X_tr, Y_tr, idxs_lb, idxs_val, net, handler, args)
elif opts.alg == 'entropy': # entropy-based sampling
    strategy = EntropySampling(X_tr, Y_tr, idxs_lb, idxs_val, net, handler, args)
elif opts.alg == 'baseline': # badge but with k-DPP sampling instead of k-means++
    strategy = BaselineSampling(X_tr, Y_tr, idxs_lb, idxs_val, net, handler, args)
elif opts.alg == 'albl': # active learning by learning
    albl_list = [LeastConfidence(X_tr, Y_tr, idxs_lb, idxs_val, net, handler, args),
        CoreSet(X_tr, Y_tr, idxs_lb, idxs_val, net, handler, args)]
    strategy = ActiveLearningByLearning(X_tr, Y_tr, idxs_lb, idxs_val, net, handler, args, strategy_list=albl_list, delta=0.1)
else: 
...
# Loop of active learning rounds
for rd in range(1, NUM_ROUND+1):
    ...
    # query from pool
    q_idxs = strategy.query(NUM_QUERY+NUM_QUERY_VAL)
    q_train = q_idxs[:NUM_QUERY]
    q_val = q_idxs[NUM_QUERY:]
    idxs_lb[q_train] = True
    idxs_val[q_val] = True

    # update learning set
    strategy.update(idxs_lb, idxs_val)
